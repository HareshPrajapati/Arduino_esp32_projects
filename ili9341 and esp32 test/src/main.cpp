...
void setup(void) {
  ...
  tft.init();
  ledcAttachPin(LED_BLE, 1);     // assign TFT_BL pin to channel 1
  ledcSetup(1, 12000UL, 8);       // 12 kHz PWM, 8-bit resolution
  ledcWrite(1, 128U);
  ...
  Serial.printf("white text \r\n");
  tft.println("White text");
  
  tft.setTextColor(TFT_RED, TFT_BLACK);
  Serial.printf("Red text \r\n");
  tft.println("Red text");

  
  tft.setTextColor(TFT_GREEN, TFT_BLACK);
  Serial.printf("Green text \r\n");
  tft.println("Green text");
  
  tft.setTextColor(TFT_BLUE, TFT_BLACK);
  Serial.printf("Blue text \r\n");
